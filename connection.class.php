...
class Common
{
	...
	public function getAttendanceData($day, $month, $year)
	{
		...
		$date_formated = date('Y-m-d', strtotime($fullDate));
		$dateis_s=$date_formated." 00:00:00";
		$dateis_e=$date_formated." 23:59:59";
		if ($_SESSION['user_type'] == 1) {
			
			$sql = "SELECT t1.*,t2.rfid_uid,min(punch_time) as punchin,max(punch_time) as punchout,t2.name 
			FROM tbl_attendance as t1 
			inner join tbl_users as t2 
			on t1.rfid_uid=t2.rfid_uid 
			WHERE punch_time between '$dateis_s' AND '$dateis_e'
			and t2.rfid_uid='" . $_SESSION['rfid_uid'] . "'";
			$retData = $this->getData($sql);
		} else if ($_SESSION['user_type'] == 2) {
			$sql = "SELECT t1.*,t2.rfid_uid,min(punch_time) as punchin,max(punch_time) as punchout,t2.name FROM tbl_attendance as t1 inner join tbl_users as t2 on t1.rfid_uid=t2.rfid_uid 
			WHERE punch_time between '$dateis_s' AND '$dateis_e'
			group by t2.rfid_uid";
			$retData = $this->getData($sql);
		}
		return $retData;
	}
